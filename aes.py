...
def subBytes(A):
  s_box = np.load('tables/s_box.npy')
...
def invSubBytes(A):
  inv_s_box = np.load('tables/inv_s_box.npy')
  B = np.zeros((4,4),dtype=int)
  for row in range(4):
    for col in range(4):
      sub_row, sub_col = A[row,col]//16, A[row,col]%16
...
def mixCol(A):
  e_table = np.load('tables/E_Table.npy')
  B = np.zeros((4,4),dtype=int)
  for row in range(4):
    for col in range(4):
      sub_row , sub_col = A[row,col]//16,A[row,col]%16
...
def invMixCol(A):
  l_table = np.load('tables/L_Table.npy')
  B = np.zeros((4,4),dtype=int)
  for row in range(4):
    for col in range(4):
      sub_row , sub_col = A[row,col]//16,A[row,col]%16
...
def aesEncrypt(plain_text,key):
    ...
    for sub_string in plain_text_split : 
        A0 = text_to_unicode(sub_string)
        A1 = subBytes(A0)
        A2 = shiftRows(A1)
        A3 = mixCol(A2)
        A4 = addRoundKey(A3,key)
        cipher_text += unicode_to_text(A4)
    return cipher_text
...
def aesDecrypt(cipher_text,key):
    ...
    for sub_string in cipher_text_split:
        cipher_text = text_to_unicode(sub_string)
        A3 = removeRoundKey(cipher_text,key)
        A2 = invMixCol(A3)
        A1 = invShiftRows(A2)
        A0 = invSubBytes(A1)
        decrypted_text+=unicode_to_text(A0)
    return decrypted_text

if __name__== '__main__':
    plain_text = input("Enter a string to encode : ")
